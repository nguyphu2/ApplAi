...
def test_overwrite_runs_three_stage_chain_and_saves_result(handler_module, monkeypatch):
    upload_test_resume(handler_module, 'user-123', 'r1', ['Ada Lovelace', 'Built REST APIs in Python'])
    invoke_model = MagicMock(side_effect=make_three_stage_responses(
        rewrite_rewrites=[{'paragraph_index': 1, 'new_text': 'Built REST APIs in Python and Flask, cutting response time by 30%'}],
    ...
    assert paragraphs[1] == 'Built REST APIs in Python and Flask, cutting response time by 30%'
...
def test_save_as_new_copy_creates_new_entry_without_touching_original(handler_module, monkeypatch):
    upload_test_resume(handler_module, 'user-123', 'r1', ['Ada Lovelace', 'Built REST APIs in Python'])
    invoke_model = MagicMock(side_effect=make_three_stage_responses(
        rewrite_rewrites=[{'paragraph_index': 1, 'new_text': 'Built scalable REST APIs in Python and Flask'}],
    ))
    monkeypatch.setattr(handler_module, 'bedrock_runtime', MagicMock(invoke_model=invoke_model))
    ...
    assert read_docx_paragraphs(original_stored['Body'].read())[1] == 'Built REST APIs in Python'


def test_bedrock_failure_returns_502(handler_module, monkeypatch):
    upload_test_resume(handler_module, 'user-123', 'r1', ['Ada Lovelace', 'Built REST APIs'])
...
def test_second_stage_failure_leaves_original_resume_untouched(handler_module, monkeypatch):
    ...
    assert read_docx_paragraphs(stored['Body'].read()) == ['Ada Lovelace', 'Built REST APIs']
    item = handler_module.table.get_item(Key={'user_id': 'user-123'})['Item']
